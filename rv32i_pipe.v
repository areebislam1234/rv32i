...
module rv32i_pipe (
    ...
    wire ex_is_load  = (ex_op == OP_LOAD);
    wire ex_is_store = (ex_op == OP_STORE);
    ...
    wire ex_reg_we = ex_writes_rd && (ex_rd != 5'd0);
    ...
    always @* begin
        ex_redirect = 1'b0;
        ex_target   = 32'b0;
        if (ex_op == OP_JAL)       begin ex_redirect=1'b1; ex_target = ex_pc + ex_imm_j; end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            ex_instr <= NOP; ex_pc <= 32'b0; ex_rs1v <= 32'b0; ex_rs2v <= 32'b0;
